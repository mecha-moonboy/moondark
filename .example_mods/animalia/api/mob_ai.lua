...
function animalia.action_swim(self, time, depth, speed, animation, pos2)
	local timeout = time or 3
	local speed_factor = speed or 0.5
	local anim = animation or "swim"
	local depth_factor = depth or 4
	...
	local function func(mob)
		...
		if not steer_direction then
			...
			if wander_point.y > depth_factor then
				wander_point.y = depth_factor
			end
	...
	local function func(mob)
		if not mob:get_action() then
			...
			or random(move_chance) < 2 then
				animalia.action_swim(mob, 0.5, 40)
			else
