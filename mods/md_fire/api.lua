...
-- add the breathable group to a node/item/tool
...
function md_fire.check_can_ignite(pos)
    ...
    -- if there isn't a fuel block nearby, cancel
    if not md_fire.node_near_fuel(air_near) then return false end

    return true
end

-- Get the flammable nodes from the 26 surrounding.
